...
//    std::cout << "Sum of activation = " << sum_of_activation << std::endl;
    std::vector<float> gt{-0.0042365542612969875, 2.214078664779663, -1.181681752204895, 0.0500468909740448, -0.4236554205417633, 0.1948026418685913,  4.421022891998291, 94.388671875, 0.0061254040338099, 97.9652099609375, 59.38797378540039};
    print_vector(gt);
    int counter_gt = 0;
    for(auto it: gt) {
...
//            std::cout << output2[5] << std::endl;
    ...
    print_vector(my_network.sum_of_gradients);
//    std::cout << "Sum of activation = " << sum_of_activation << std::endl;
    std::vector<float> gt{-3.9184300899505615, 9.505999565124512, -45.94569778442383, 93.51499938964844, -391.843017578125, 529.8500366210938, 4851.0, 587.7557373046875, 3001.9609375, 318.23748779296875, 273.9898681640625, 611.520263671875};
    int counter_gt = 0;
    for(auto it: gt) {
        if (std::abs(it - my_network.sum_of_gradients[counter_gt]) > 1e-3) {
            return false;
